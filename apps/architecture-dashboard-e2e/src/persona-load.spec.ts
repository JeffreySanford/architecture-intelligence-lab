...
test('selecting Adhan Designer does not leave stale loading personas state', async ({ page }) => {
  ...
  await expect(page.locator('button:has-text("Enter As Adhan Designer")')).toBeVisible({ timeout: 30000 });
...
test('selecting Alice Viewer does not leave stale loading personas state', async ({ page }) => {
  ...
  const selectResponsePromise = page.waitForResponse((response) =>
    response.url().includes('/api/dev-auth/personas/alice-viewer/select') && response.status() === 200,
  );
